namespace Loupedeck.MouseTronPlugin
{
    ...
    public class SecondMostUsedAction : PluginDynamicCommand
    {
        ...
        public static void UpdateRecent()
        {
            try
            {
                ...
                {
                    var jsonPath = System.IO.Path.Combine(parentDirectory, "recommendations/stable_tools_combo_2.json"); // Assuming file name is recent.json
        ...
        public SecondMostUsedAction()
            : base(displayName: "Second Most Used Action", description: "The second most used action by user", groupName: "HTTP Actions")
        {
            UpdateRecent();
        }
        ...
        private async Task ExecuteAsync()
        {
            try
            {
                ...
                var userInput = SystemInfo.ShowInputDialog("Enter Input", $"The second most used action is: {com_description}\n" +
        ...
        // Reuses the same icon as FirstMostUsedAction
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
            ...
            {
                PluginLog.Warning($"Failed to load icon for SecondMostUsedAction: {ex.Message}");
                return null;
            }
        }
