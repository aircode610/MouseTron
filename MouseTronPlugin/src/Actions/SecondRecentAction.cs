namespace Loupedeck.MouseTronPlugin
{
    ...
    public class SecondRecentAction : PluginDynamicCommand
    {
        ...
        public static void UpdateRecent()
        {
            try
            {
                ...
                {
                    var jsonPath = System.IO.Path.Combine(parentDirectory, "recommendations/recent_tool_single_2.json"); // Assuming file name is recent.json
        ...
        public SecondRecentAction()
            : base(displayName: "Second Most Recent Action", description: "The most recent action done by user", groupName: "HTTP Actions")
        {
            UpdateRecent();
        }
        ...
        private async Task ExecuteAsync()
        {
            try
            {
                ...
                var userInput = SystemInfo.ShowInputDialog("Enter Input", $"The second most recent action is: {com_description}\n" +
        ...
        // Reuses the same icon as FirstRecentAction
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
            ...
            {
                PluginLog.Warning($"Failed to load icon for SecondRecentAction: {ex.Message}");
                return null;
            }
        }
